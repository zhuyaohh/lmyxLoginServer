...
public class HttpRequest {
	 ...
	 * @return URL 所代表远程资源的响应结果
	 */
	public static String sendOMGet(String url, String param) {
		String result = "";
		BufferedReader in = null;
		try {
			String urlNameString = url + "&" + param;
	...
	public static String sendPost(String url, byte[] param) {
		...
		try {
			...
			out.flush();
			// 定义BufferedReader输入流来读取URL的响应
			result = _gzipStream2Str(conn.getInputStream());
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 使用finally块来关闭输出流、输入流
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < 10; i++) {
			...
			/*
			 * req=2&id=user123123&password=123123&mail=13600717@qq.com
			 */
			String gameid = "2951";
			String gamepw = "yHd#4$$58aF";
			logger.info("第" + (i + 1) + "次请求启动");
			StringBuffer contents = new StringBuffer();
			contents.append("gameid=" + gameid);
			contents.append("&gamepw=" + gamepw);
			contents.append("&req=" + 2);
			contents.append("&id=" + "user1342");
			contents.append("&password=" + "tea987");
			contents.append("&mail=" + "ddd@yeah.net");
			String result = HttpRequest.sendPost(url, contents.toString());
			logger.info("第" + (i + 1) + "次请求结果:" + result);
		}
